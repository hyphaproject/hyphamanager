...
DoorOpenerWidget::DoorOpenerWidget(QWidget *parent)
    : QWidget(parent), ui(new Ui::DoorOpenerWidget) {
  ui->setupUi(this);
}

DoorOpenerWidget::~DoorOpenerWidget() { delete ui; }

void DoorOpenerWidget::setDatabase(hypha::database::Database *database) {
  this->database = database;
}

void DoorOpenerWidget::setUserDatabase(
    hypha::database::UserDatabase *userDatabase) {
  this->userDatabase = userDatabase;
}

void DoorOpenerWidget::setId(QString id) { this->id = id; }
...
void DoorOpenerWidget::reloadUser() {
  ...
  statement << "select user,atworktime from dooropener_user where id='" +
                   id.toStdString() + "'";
  statement.execute();
...
QString DoorOpenerWidget::getConfig() {
  return "{\"mastercard\":\"" + ui->masterCardEdit->text() + "\"" +
         ",\"email\":" +
         (ui->emailCheckBox->isChecked() ? QString("true") : QString("false")) +
         ",\"fingerprint\":" +
         (ui->fingerprintCheckBox->isChecked() ? QString("true")
                                               : QString("false")) +
         "}";
}
...
void DoorOpenerWidget::on_deleteButton_clicked() {
  QString user =
      this->listModel.itemFromIndex(ui->listView->currentIndex())->text();

  Poco::Data::Statement statement = database->getStatement();
  statement << "DELETE FROM dooropener_user WHERE `id`='" + id.toStdString() +
                   "' AND `user`='" + user.toStdString() + "';";
  statement.execute();
...
void DoorOpenerWidget::on_addButton_clicked() {
  ...
  bool ok;
  QString user = QInputDialog::getItem(this, tr("Add User"), tr("User:"), users,
                                       0, false, &ok);
  if (ok && !user.isEmpty()) {
    ...
    Poco::Data::Statement statement = database->getStatement();
    statement
        << "insert into dooropener_user(id,user,atworktime) values(?, ?, ?);",
        Poco::Data::Keywords::use(idStr), Poco::Data::Keywords::use(userStr),
        Poco::Data::Keywords::use(atworktime);
    statement.execute();
    reloadUser();
  }
}
