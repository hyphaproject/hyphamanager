...
Translation *Translation::instance() {
  ...
  if (!singleton) {
    mutex.lock();
    if (!singleton) singleton = new Translation(0);
...
void Translation::loadLanguage() {
  ...
  this->currentLanguage =
      settings.value(QString::fromUtf8("language"), "english").toString();
}
...
QString Translation::language() { return this->currentLanguage; }

QString Translation::locale() { return tr("en"); }
...
void Translation::loadTranslations(QString dir) {
...
#ifdef Q_OS_WIN32
    if (!translationsDir.cd("i18n")) return;
#else
    if (!translationsDir.cd("../i18n")) return;
#endif
  }
  foreach (QString qmFile, translationsDir.entryList(QStringList() << "*.qm")) {
    if (!languageName(translationsDir.absoluteFilePath(qmFile)).isEmpty()) {
      languages.insert(languageName(translationsDir.absoluteFilePath(qmFile)),
                       translationsDir.absoluteFilePath(qmFile));
    }
  }
}
