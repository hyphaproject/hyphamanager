...
void NewConfigWizzard::on_selectFileButton_clicked() {
    QString fileName = QFileDialog::getSaveFileName(this,
                       tr("Config File"), ui->configFileEdit->text(), tr("Hypha Config File (*.xml)"));
...
void NewConfigWizzard::on_cancelButton_clicked() {
    this->reject();
}
...
void NewConfigWizzard::on_nextButton_clicked() {
    if (ui->tabWidget->currentIndex() == ui->tabWidget->count() - 1) {
        ...
        generator.udbAttributeFirstname = ui->udAttrFirstnameEdit->text().toStdString();
        generator.udbAttributeLastname = ui->udAttrLastnameEdit->text().toStdString();
        generator.udbAttributeMail = ui->udAttrMailEdit->text().toStdString();
        generator.udbAttributeUsername = ui->udAttrUsernameEdit->text().toStdString();
