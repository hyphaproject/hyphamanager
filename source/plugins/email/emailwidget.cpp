...
EMailWidget::EMailWidget(QWidget *parent)
    : QWidget(parent), ui(new Ui::EMailWidget) {
  ui->setupUi(this);
}

EMailWidget::~EMailWidget() { delete ui; }

void EMailWidget::setHost(QString host) { this->host = host; }
...
QString EMailWidget::getConfig() {
  return "{\"user\":\"" + ui->userEdit->text() + "\"" + ",\"password\":\"" +
         ui->passwordEdit->text() + "\"" + ",\"recipient\":\"" +
         ui->recipientEdit->text() + "\"" + ",\"host\":\"" +
         ui->hostEdit->text() + "\"" + ",\"port\":" +
