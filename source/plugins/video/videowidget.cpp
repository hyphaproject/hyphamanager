...
void VideoWidget::setHost(QString host) { this->host = host; }
...
void VideoWidget::loadConfig(QString json) {
  ...
  if (object.contains("devices")) {
    ...
    ui->video0UpsideDownCheckBox->setChecked(
        video0.value("upsidedown").toBool());
    QJsonObject video1 = devices.at(1).toObject();
    ui->video1DeviceEdit->setText(video1.value("device").toString());
    ui->video1UpsideDownCheckBox->setChecked(
        video1.value("upsidedown").toBool());
    QJsonObject video2 = devices.at(2).toObject();
    ui->video2DeviceEdit->setText(video2.value("device").toString());
    ui->video2UpsideDownCheckBox->setChecked(
        video2.value("upsidedown").toBool());
    QJsonObject video3 = devices.at(3).toObject();
    ui->video3DeviceEdit->setText(video3.value("device").toString());
    ui->video3UpsideDownCheckBox->setChecked(
        video3.value("upsidedown").toBool());
  }

  ui->webView->settings()->setAttribute(QWebSettings::JavascriptEnabled, true);
  ui->webView->settings()->setAttribute(QWebSettings::JavascriptCanOpenWindows,
                                        true);
  ...
  ui->webView->load(
      QUrl("http://" + host + ":" + QString::number(ui->portSpinBox->value())));
  ui->urlLabel->setText("<a href=\"http://" + host + ":" +
                        QString::number(ui->portSpinBox->value()) +
                        "\">Video Stream http://" + host + ":" +
                        QString::number(ui->portSpinBox->value()) + "</a>");
}
