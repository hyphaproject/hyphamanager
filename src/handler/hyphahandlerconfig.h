...
public:
    ...
    std::string communicate(std::string message){return "";}
    virtual HyphaHandlerConfig * getInstance(std::string id) = 0;
    std::string getHost(){ return host; }
    void setHost(std::string host){ this->host = host; }
    void setUserDatabase(hypha::database::UserDatabase * userDatabase){ this->userDatabase = userDatabase; }
    void setDatabase(hypha::database::Database * database){ this->database = database; }
