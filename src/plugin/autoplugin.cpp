...
const std::string AutoPlugin::name()
{
    return plugin->name();
}

const std::string AutoPlugin::getTitle()
{
    return plugin->getTitle();
}

const std::string AutoPlugin::getVersion()
{
...
std::string AutoPlugin::getConfig()
{
    return pluginWidget->getConfig();
}

HyphaPluginConfig *AutoPlugin::getInstance(std::string id)
{
    return nullptr;
}
...
{
    if (!pluginWidget)
        pluginWidget = new AutoPluginWidget(plugin);
    return pluginWidget;
}
